...
async def get_available_slots(
    ...
    # Get Session
    session_result = await db.execute(select(ExamSession).where(ExamSession.id == exam.session_id))
    ...
    rooms_query = select(ExamRoom).where(
        ExamRoom.exam_capacity >= len(student_ids),
        ExamRoom.is_active == True,
        ExamRoom.is_available == True
    ).order_by(ExamRoom.exam_capacity) # Use smallest fitting room first
...
async def schedule_single_exam(
    ...
    """
    # Get available slots
    slots = await get_available_slots(exam_id, db=db, limit=1, current_user=current_user)
...
async def schedule_entire_session(
    ...
    for exam in pending_exams:
        ...
        if exam.requires_lab:
            ...
            candidate_rooms = [r for r in computer_rooms if r.exam_capacity >= student_count]
        else:
            candidate_rooms = [r for r in rooms_by_capacity if r.exam_capacity >= student_count]
...
async def prepare_session_for_scheduling(
    ...
    modules_res = await db.execute(
        select(Module).join(Formation).where(
            Formation.academic_year == session.academic_year,
...
async def clear_session_schedule(
    ...
    """Python implementation of clear session"""
    result = await db.execute(
    ...
    # Delete supervisors for this session's exams
    from app.models import ExamSupervisor
    exam_ids = [ex.id for ex in exams]
    if exam_ids:
        from sqlalchemy import delete
        await db.execute(delete(ExamSupervisor).where(ExamSupervisor.exam_id.in_(exam_ids)))
    
    await db.commit()
    return {
        "message": f"Cleared schedule for {count} exams",
        "exams_cleared": count
    }
...
async def assign_exam_supervisors(
    ...
    assignments_res = await db.execute(select(ExamSupervisor).where(ExamSupervisor.exam_id.in_([e.id for e in exams])))
    ...
    return {
        ...
        "avg_supervisions": sum(prof_load.values()) / len(professors) if professors else 0
    }
...
async def get_session_statistics(
    ...
    scheduled = await db.scalar(
        select(func.count(Exam.id)).where(Exam.session_id == session_id, Exam.status == 'scheduled')
    )
    pending = (total or 0) - (scheduled or 0)
    
    # Rooms used
    rooms = await db.scalar(
        select(func.count(func.distinct(Exam.room_id))).where(Exam.session_id == session_id, Exam.room_id != None)
    )
...
async def get_schedule_conflicts(
    ...
    """
    from app.models import Exam, Module, ExamRoom, ExamSupervisor, Professor, Enrollment, Student
    ...
    def times_overlap(start1, dur1, start2, dur2):
        if not start1 or not start2: return False
    ...
    for i, row1 in enumerate(exams_rows):
        ...
        for j, row2 in enumerate(exams_rows):
            ...
            if e1.room_id == e2.room_id and e1.scheduled_date == e2.scheduled_date:
                if times_overlap(e1.start_time, e1.duration_minutes, e2.start_time, e2.duration_minutes):
                    conflicts.append({
                        "type": "Room Overlap",
                        "severity": "Critical",
                        "item": f"{r1_name}",
    ...
    sup_query = (
        select(ExamSupervisor, Professor.first_name, Professor.last_name, Exam.scheduled_date, Exam.start_time, Exam.duration_minutes, Module.name)
        ...
        for j, s2 in enumerate(sups):
            ...
            if s1.ExamSupervisor.professor_id == s2.ExamSupervisor.professor_id:
                if s1.scheduled_date == s2.scheduled_date:
                    if times_overlap(s1.start_time, s1.duration_minutes, s2.start_time, s2.duration_minutes):
                        conflicts.append({
    ...
    for row in exams_rows:
        e, name, _, _ = row
        mod_timing[e.module_id] = (e.scheduled_date, e.start_time, e.duration_minutes, name)

    # 2. Get All Enrollments for active students
    enr_query = (
        select(Enrollment.student_id, Enrollment.module_id, Student.first_name, Student.last_name)
