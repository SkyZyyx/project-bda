...
from utils.styles import inject_custom_css, page_header, metric_card, conflict_indicator
from utils.api import api, is_authenticated, get_current_user, logout
...
with st.sidebar:
    ...
    if is_authenticated():
        ...
        if user_role in ["admin"]:
            ...
            menu_options = ["Dashboard", "Scheduling", "Exams", "Departments", "Professors", "Personal Schedule", "Settings"]
            menu_icons = ["grid-fill", "calendar-check-fill", "journal-bookmark-fill", "building-fill", "person-badge-fill", "person-circle", "gear-fill"]
        elif user_role in ["dean", "vice_dean"]:
            # Doyen/Vice-Doyen: vue stratégique, validation, pas de planning manuel
            menu_options = ["Dashboard", "Validation", "Exams", "Departments", "Professors", "Settings"]
            menu_icons = ["grid-fill", "check-circle-fill", "journal-bookmark-fill", "building-fill", "person-badge-fill", "gear-fill"]
        elif user_role in ["department_head", "dept_head"]:
            # Chef département: validation de son département
            menu_options = ["Dashboard", "Validation", "Exams", "My Department", "Personal Schedule", "Settings"]
            menu_icons = ["grid-fill", "check-circle-fill", "journal-bookmark-fill", "building-fill", "person-circle", "gear-fill"]
...
if not is_authenticated():
    ...
    with col2:
        st.markdown(page_header(
            "🎓 Welcome Back",
            "Sign in to access the Exam Scheduling Platform"
        ), unsafe_allow_html=True)
    ...
    if stats.get("error"):
        ...
        progress_pct = int(scheduled_exams / total_exams * 100) if total_exams > 0 else 0
        ...
        with col1:
            st.markdown(metric_card(
                ...
                "Total Étudiants",
                "👨‍🎓",
                trend=f"{stats.get('total_formations', 0)} formations", trend_up=True
            ), unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(metric_card(
                ...
                "Total Examens",
                "📚",
                trend=f"{pending_exams} en attente", trend_up=pending_exams == 0
            ), unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(metric_card(
                ...
                "Progression",
                "🗓️",
                trend=f"{progress_pct}% planifiés", trend_up=progress_pct > 50
            ), unsafe_allow_html=True)
        ...
        with col4:
            st.markdown(metric_card(
                ...
                "Conflits",
                "⚠️",
                trend="Action requise" if conflicts > 0 else "Aucun conflit", trend_up=conflicts == 0
            ), unsafe_allow_html=True)
        ...
        with col_main:
            ...
            if recent_exams and not (isinstance(recent_exams, dict) and recent_exams.get("error")):
                import pandas as pd
                df = pd.DataFrame(recent_exams)
                if not df.empty:
                    # Map IDs to names if needed, or use what's available
                    display_cols = ["module_name", "scheduled_date", "room_name", "status"]
                ...
                st.info("Quick schedule some exams in the 'Scheduling' tab to see data here.")
    ...
    with st.spinner("Analyzing session state..."):
        ...
        active_sessions = overview.get("active_sessions", []) if not overview.get("error") else []
    
    if not active_sessions:
        st.warning("⚠️ No active exam sessions found. Please initialize a session in the database first.")
    else:
        ...
        with col_sel:
            selected_name = st.selectbox("Current Active Session", options=session_map.keys())
            selected_id = session_map[selected_name]
            # Get specific session details for better metrics
            curr_session = next((s for s in active_sessions if s["id"] == selected_id), {})
        ...
        with kpi4:
            st.markdown(metric_card(
                ...
                "Taux Planification", 
                "📊", 
                trend="Complet" if progress_pct == 100 else f"{pending_exams} restants",
        ...
        tab1, tab2, tab3 = st.tabs(["🤖 Auto-Resolve", "📋 Manual Override", "⚠️ Conflict Report"])
            ...
            with c1:
                ...
                if st.button("Initialize Exams", use_container_width=True):
                    with st.spinner("Processing modules..."):
                        ...
                        if res.get("error"): st.error(res.get("detail"))
                        else: st.success("✅ Registry Initialized"); st.rerun()
            
            with c2:
                st.markdown("**Phase 2: Optimization**")
                st.caption("Heuristic slot placement")
                if st.button("Launch Auto-Schedule", type="primary", use_container_width=True):
                    with st.spinner("Running Optimization Engine..."):
                        res = api.post(f"/scheduling/schedule-session/{selected_id}")
                        if res.get("error"): st.error(res.get("detail"))
                        else: st.success("✅ Schedule Generated"); st.rerun()
            ...
            with c3:
                ...
                if st.button("Assign Supervisors", use_container_width=True):
                    with st.spinner("Optimizing staff assignments..."):
                        ...
                        if res.get("error"): st.error(res.get("detail"))
                        else: st.success("✅ Staff Assigned"); st.rerun()

            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🗑️ Clear All Current Schedules", key="clear_sess", help="Reset all mappings for this session"):
                res = api.post(f"/scheduling/clear-session/{selected_id}")
                if not res.get("error"): st.success("Session reset."); st.rerun()
        ...
        with tab2:
            ...
            if isinstance(exams_list, list) and isinstance(rooms_list, list):
                exam_options = {f"{e['module_code']} - {e['module_name']}": e['id'] for e in exams_list}
                room_options = {f"{r['name']} ({r['building']}) Cap:{r['exam_capacity']}": r['id'] for r in rooms_list}
                
                with st.form("manual_edit"):
                    f1, f2 = st.columns(2)
                    with f1:
                        target_exam = st.selectbox("Select Target Exam", options=list(exam_options.keys()))
                        target_room = st.selectbox("Assign to Room", options=list(room_options.keys()))
                    with f2:
                        target_date = st.date_input("Schedule Date")
                        target_time = st.time_input("Start Time")
                    
                    if st.form_submit_button("Apply Correction", use_container_width=True, type="primary"):
                        payload = {
                        ...
                        if not res.get("error"): st.success("✅ Change applied!"); st.rerun()
        ...
        with tab3:
            ...
            if conflicts and isinstance(conflicts, list) and len(conflicts) > 0:
                ...
                st.error(f"Found {len(conflicts)} integrity issues that require resolution.")
            else:
                st.success("✨ Physical and logical integrity checks passed! No conflicts found.")
    ...
    with f1:
        status_filter = st.selectbox("Exam Status", ["All", "Scheduled", "Pending", "Completed"])
    with f2:
        ...
        if isinstance(depts_res, list):
            for d in depts_res: dept_options[d['name']] = d['id']
        selected_dept_label = st.selectbox("Department", options=list(dept_options.keys()))
    with f3:
        search = st.text_input("Search module name, code or room...", placeholder="e.g. Algorithms")
    with f4:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    with exp:
        ...
        with st.form("quick_create"):
            ...
            with q1:
                ...
                sel_session = st.selectbox("Exam Session", options=list(s_map.keys()) if s_map else ["N/A"])
                m_code = st.text_input("Module Code identifier", placeholder="e.g. CS101")
            with q2:
            ...
            if st.form_submit_button("Finalize Registration", use_container_width=True):
                st.warning("Manual registration currently requires verified module codes. Please use Auto-Initialize.")
    
    # List Layout
    with st.spinner("Retrieving registry..."):
        params = {}
        if status_filter != "All": params["status"] = status_filter.lower()
        if search: params["search"] = search
        if dept_options[selected_dept_label]: params["department_id"] = dept_options[selected_dept_label]
    ...
    if exams and isinstance(exams, list):
        ...
        if not df.empty:
            ...
            cols = ["module_code", "module_name", "department_name", "scheduled_date", "room_name", "status"]
    ...
    st.markdown(page_header("Academic Departments", "Departmental hierarchy and building allocation"), unsafe_allow_html=True)
    with st.spinner("Loading structures..."):
        depts = api.get("/departments")
        if depts and isinstance(depts, list):
            import pandas as pd
            st.dataframe(pd.DataFrame(depts)[["name", "code", "building"]], use_container_width=True, hide_index=True)

elif selected == "Professors":
    st.markdown(page_header("Faculty Management", "Professor profiles and supervision workload limits"), unsafe_allow_html=True)
    
    # Simple search & Add
    col_search, col_add = st.columns([2, 1])
    with col_search:
        search_prof = st.text_input("🔍 Search Faculty", placeholder="Name, department or email...")
    with col_add:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    if show_add:
        with st.form("add_professor_premium"):
            ...
            with c2:
                depts = api.get("/departments")
                dept_map = {d['name']: d['id'] for d in depts} if isinstance(depts, list) else {}
                p_dept = st.selectbox("Assign to Department", options=list(dept_map.keys()))
                p_title = st.selectbox("Academic Title", ["Professor", "Associate Professor", "Assistant Professor", "Lecturer"])
            
            if st.form_submit_button("Finalize Registration", type="primary", use_container_width=True):
                if p_first and p_last and p_email:
                    ...
                    if not res.get("error"): st.success("✅ Registered!"); st.rerun()
                    else: st.error(res.get("detail"))
                else:
    ...
    if profs and isinstance(profs, list):
        ...
        if not df.empty:
            st.markdown(f'<div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.5rem; font-weight: 600;">ACTIVE FACULTY ({len(df)})</div>', unsafe_allow_html=True)
    ...
    if student_id: 
        ...
        st.markdown(f'<div style="margin-bottom: 1rem; color: var(--primary); font-weight: 700;">📘 STUDENT MODE • ID: {student_id[:8]}...</div>', unsafe_allow_html=True)
    elif prof_id: 
        params["professor_id"] = prof_id
        st.markdown(f'<div style="margin-bottom: 1rem; color: var(--primary); font-weight: 700;">🛡️ PROFESSOR MODE • ID: {prof_id[:8]}...</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div style="margin-bottom: 1rem; color: var(--primary); font-weight: 700;">⚙️ ADMIN MODE • FULL ACCESS</div>', unsafe_allow_html=True)
    ...
    st.markdown(page_header(
        "🏫 My Department",
        f"Vue d'ensemble et gestion du département"
    ), unsafe_allow_html=True)
    
    if not dept_id:
        st.error("⚠️ Votre compte n'est associé à aucun département. Contactez l'administrateur.")
    else:
        ...
        if isinstance(stats, dict) and not stats.get("error"):
            ...
            with k1:
                st.markdown(metric_card(str(stats.get("formations_count", 0)), "Formations", "🎓", "Offres actives"), unsafe_allow_html=True)
            with k2:
                st.markdown(metric_card(str(stats.get("total_students", 0)), "Étudiants", "👥", "Inscrits"), unsafe_allow_html=True)
            with k3:
                st.markdown(metric_card(str(stats.get("professors_supervising", 0)), "Professeurs", "👨‍🏫", "Superviseurs"), unsafe_allow_html=True)
            with k4:
                st.markdown(metric_card(str(stats.get("total_exams", 0)), "Examens", "📝", "Session active"), unsafe_allow_html=True)
            
            st.markdown("---")
            
            # Formations List
            st.subheader("🎓 Formations du Département")
            formations = api.get(f"/departments/{dept_id}/formations") # Hypothetical endpoint or filter
            ...
            if isinstance(all_formations, list):
                my_formations = [f for f in all_formations if f.get('department_id') == dept_id]
                if my_formations:
                    import pandas as pd
                    df_form = pd.DataFrame(my_formations)
                    st.dataframe(df_form[["name", "code", "level"]], use_container_width=True, hide_index=True)
                else:
    ...
    with tab1:
        ...
        with st.form("profile_form"):
            name = st.text_input("Full Name", value=user.get("name", "") if user else "")
    ...
    if not student_id:
        st.warning("⚠️ Votre compte n'est pas lié à un profil étudiant. Contactez l'administration.")
    else:
        st.markdown(f"""
        ...
        with st.spinner("Chargement de vos examens..."):
            my_exams = api.get("/exams", {"student_id": student_id, "status": "scheduled"})
        
        if my_exams and isinstance(my_exams, list) and len(my_exams) > 0:
            st.markdown(f"<div style='margin-bottom: 1rem; font-weight: 600;'>📋 {len(my_exams)} examen(s) à passer</div>", unsafe_allow_html=True)
    ...
    if not prof_id:
        st.warning("⚠️ Votre compte n'est pas lié à un profil professeur. Contactez l'administration.")
    else:
        st.markdown(f"""
        ...
        with st.spinner("Chargement de vos surveillances..."):
            my_supervisions = api.get("/exams", {"professor_id": prof_id, "status": "scheduled"})
        
        if my_supervisions and isinstance(my_supervisions, list) and len(my_supervisions) > 0:
            st.markdown(f"<div style='margin-bottom: 1rem; font-weight: 600;'>📋 {len(my_supervisions)} surveillance(s) assignée(s)</div>", unsafe_allow_html=True)
    ...
    if user_role in ["dean", "vice_dean", "dept_head", "department_head"]:
        ...
        if overview and not overview.get("error"):
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.markdown(metric_card(str(overview.get("total_departments", 0)), "Départements", "🏛️"), unsafe_allow_html=True)
            with col2:
                st.markdown(metric_card(str(overview.get("total_students", 0)), "Étudiants", "👨‍🎓"), unsafe_allow_html=True)
            with col3:
                st.markdown(metric_card(str(overview.get("total_professors", 0)), "Professeurs", "👨‍🏫"), unsafe_allow_html=True)
            with col4:
                sessions = overview.get("active_sessions", [])
                total_exams = sum(s.get("total_exams", 0) for s in sessions)
                st.markdown(metric_card(str(total_exams), "Total Examens", "📋"), unsafe_allow_html=True)
        ...
        if dept_id:
            with st.spinner("Chargement des examens du département..."):
                dept_exams = api.get("/exams", {"department_id": dept_id, "status": "scheduled"})
            
            if dept_exams and isinstance(dept_exams, list):
                st.markdown(f"<div style='margin-bottom: 1rem; font-weight: 600;'>📋 {len(dept_exams)} examen(s) dans votre département</div>", unsafe_allow_html=True)
                ...
                if st.button("✅ Valider les examens du département", type="primary", use_container_width=True):
    ...
    st.markdown(page_header(
        "🏛️ Mon Département",
        "Statistiques et gestion de votre département"
    ), unsafe_allow_html=True)
    ...
    if dept_id:
        ...
        if dept_info and not dept_info.get("error"):
            ...
            with col1:
                ...
                st.markdown(metric_card(str(len(formations)), "Formations", "📚"), unsafe_allow_html=True)
            with col2:
                exam_count = len(dept_exams) if isinstance(dept_exams, list) else 0
                st.markdown(metric_card(str(exam_count), "Examens", "📋"), unsafe_allow_html=True)
    else:
        st.warning("⚠️ Aucun département associé à votre compte.")
